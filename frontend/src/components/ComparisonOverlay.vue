<template>
  <!-- Teleport renders this outside the component tree, directly on <body> -->
  <Teleport to="body">
          ...
          <div class="overlay-body">
            ...
            <div class="products-row">
              <div v-for="product in selectedProducts" :key="product.id" class="product-thumb-card">
                ...
                <div class="thumb-meta">{{ product.brand }} · {{ product.unit }}</div>
              </div>
            </div>
              ...
              <table class="price-table">
                <thead>
                  <tr>
                    ...
                    <th v-for="p in selectedProducts" :key="p.id" class="product-col">
                ...
                <tbody>
                  <tr v-for="store in STORES" :key="store.id">
                    ...
                    <td
                      ...
                      :key="product.id"
                      :class="['price-cell', { 'is-best': isBestPrice(product, store.id) }]"
                    >
                      <template v-if="getStorePrice(product, store.id)">
                        <span class="p-main">{{ fmt(getStorePrice(product, store.id).salePrice || getStorePrice(product, store.id).regularPrice) }}</span>
                        <span v-if="getStorePrice(product, store.id).salePrice" class="p-old">{{ fmt(getStorePrice(product, store.id).regularPrice) }}</span>
...
// Consume shared state from Products.vue via provide/inject
...
const fmt = (price) => vndFmt.format(price)
...
function isBestPrice(product, storeId) {
  ...
  const effective = entry.salePrice || entry.regularPrice
  const all = product.prices.map(p => p.salePrice || p.regularPrice)
  return effective === Math.min(...all)
}
...
const chartOptions = {
  ...
  plugins: {
    ...
    tooltip: {
      callbacks: {
        label: (ctx) => `${ctx.dataset.label}: ${vndFmt.format(ctx.raw)}`
      }
    }
  },
  scales: {
